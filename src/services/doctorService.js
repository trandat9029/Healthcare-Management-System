    ...
    try {
      ...
      const users = await db.User.findAll({
        ...
        attributes: {
          ...
          include: [
            [
              Sequelize.fn("COUNT", Sequelize.col("doctorBookings.id")),
              "monthCompleteCount",
            ],
          ],
        },
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (doctorInfo) {
        ...
        doctorInfo.dateOfBirth = doctorInfoPayload.dateOfBirth;
      ...
      if (kwRaw) {
        ...
        const fullNameOr = [
          sequelizeWhere(
            fn(
              "concat",
              col("patientData.lastName"),
              " ",
              col("patientData.firstName")
            ),
            {
              [Op.like]: kwLike,
            }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (!appointment) {
        ...
        return resolve({
          errCode: 0,
          errMessage: "Already finished or not found",
        });
      }
...
let handleCancelBooked = (data) => {
  return new Promise(async (resolve, reject) => {
    const t = await db.sequelize.transaction();
    try {
      const { bookingId, doctorId, cancelReason, language } = data;
      ...
      const booking = await db.Booking.findOne({
        where: { id: bookingId, doctorId: doctorId },
        transaction: t,
        lock: t.LOCK.UPDATE,
        raw: false,
        include: [
      ...
      // chỉ cho hủy khi S1 hoặc S2
      if (!["S1", "S2"].includes(booking.statusId)) {
        if (!t.finished) await t.rollback();
        return resolve({
          errCode: 3,
          errMessage: "Booking cannot be cancelled in this status!",
        });
      }
      ...
      // CHỈ DÙNG NẾU DB CÓ CỘT cancelReason
      ...
      const doctorName = `${booking.doctorBookings?.firstName || ""} ${
        booking.doctorBookings?.lastName || ""
      }`.trim();

      const patientName = `${booking.patientData?.firstName || ""} ${
        booking.patientData?.lastName || ""
      }`.trim();

      const timeString = isVi
        ? booking.timeTypeDataPatient?.valueVi
        : booking.timeTypeDataPatient?.valueEn;

      if (!booking.patientData?.email) {
        return resolve({
          errCode: 4,
      ...
      // GỬI EMAIL. FAIL KHÔNG LÀM HỎNG API
      try {
        await emailService.sendCancelEmailByDoctor({
          receiverEmail: booking.patientData.email,
        ...
        console.log("Email failed but booking cancelled:", e);
      }

      return resolve({
        errCode: 0,
        errMessage: "Cancelled successfully",
      });
    } catch (error) {
      if (!t.finished) await t.rollback();
      return reject(error);
